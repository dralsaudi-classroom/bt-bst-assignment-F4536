...
public class BST<T> {
    ...
    // Find the node with key k and start counting from there
    Node rootOfSubtree = findNode(root, k);
    return countNodes(rootOfSubtree);
}

// Helper method to find the node with the specified key k
...
// Helper method to count nodes in the subtree rooted at the given node
